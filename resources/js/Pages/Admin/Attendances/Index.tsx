...
// ✅ number 用（in/outは number 固定の想定）
const fmtMinutes0 = (m: number) => {
  if (!m || m <= 0) return '—'
  const h = Math.floor(m / 60)
  const mm = m % 60
  return `${h}h ${mm}m`
...
export default function Index({ auth, filters, attendances }: Props) {
  ...
  return (
    <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">勤怠一覧（管理者）</h2>}>
      <Head title="勤怠一覧（管理者）" />
                ...
                <table className="min-w-full border">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="border px-3 py-2 text-xs">残業（内/外）</th>
                      <th className="border px-3 py-2 text-xs">深夜</th>
                      <th className="border px-3 py-2 text-left text-xs text-gray-600">所定内労働時間</th>
                  ...
                  <tbody>
                    {attendances.data.length === 0 ? (
                      ...
                      attendances.data.map((row) => {
                        ...
                        const otIn = row.overtime?.in ?? 0
                        const otOut = row.overtime?.out ?? 0
                        const otTotal = otIn + otOut
                        ...
                        return (
                          <tr key={row.id}>
                            ...
                            {/* ✅ 残業（法定内/法定外） */}
                            <td className="border px-3 py-2 text-sm">
                              {!row.clock_out || otTotal <= 0 ? (
                                '—'
                              ) : (
                                <div className="flex flex-col gap-1 text-xs">
                                  <div className="flex items-center justify-between gap-2">
                                    <span className="text-gray-600">内</span>
                                    <span className="text-gray-800">{fmtMinutes0(otIn)}</span>
                                  </div>
                                  <div className="flex items-center justify-between gap-2">
                                    <span className="text-gray-600">外</span>
                                    <span className="font-medium text-red-600">{fmtMinutes0(otOut)}</span>
                                  </div>
                                </div>
                              )}
                            </td>

                            <td className="border px-3 py-2 text-sm">{fmtMinutes(row.night_minutes ?? null)}</td>
                            <td className="border px-3 py-2 text-sm">{fmtMinutes(row.scheduled_work_minutes)}</td>
                                  ...
                                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                                    ...
                                    <div className="rounded-md border p-3">
                                      ...
                                      <div className="mt-2 flex gap-2">
                                        <button
                                          type="button"
                                          onClick={() => setClockOutDateDraft(addDaysYMD(clockOutDateDraft || row.work_date, 1))}
