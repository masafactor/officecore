...
@foreach ($reports as $report)
    @php
        ...
        $attendances = $report['rows'] ?? [];
        $userName = $report['user']->name ?? '—';
        $year = $report['year'] ?? '—';
        $month = $report['month'] ?? '—';

        $requestStatus = $report['request_status'] ?? 'approved';
        $requestStatusLabel = $statusMap[$requestStatus] ?? $requestStatus;

        $submittedAt = $report['submitted_at'] ?? null;
        $approvedAt = $report['approved_at'] ?? null;
        $approverName = $report['approver_name'] ?? null;
