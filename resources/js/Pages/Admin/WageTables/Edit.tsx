...
export default function Edit({ wageTable, employmentTypes }: Props) {
  const form = useForm({
    employment_type_id: String(wageTable.employment_type_id),
    code: wageTable.code ?? '',
    name: wageTable.name ?? '',
    hourly_wage: String(wageTable.hourly_wage ?? ''),
    start_date: wageTable.start_date ?? '',
    end_date: wageTable.end_date ?? '',
  })

  const submit = (e: React.FormEvent) => {
    e.preventDefault()

    form.put(route('admin.wage-tables.update', wageTable.id), {
      preserveScroll: true,
    })
  }
  ...
  return (
    <AuthenticatedLayout
      header={<h2 className="text-xl font-semibold leading-tight text-gray-800">賃金テーブル編集</h2>}
    >
      <Head title="賃金テーブル編集" />
          ...
          <div className="overflow-hidden rounded-xl bg-white shadow-sm">
            <div className="p-6 md:p-8">
              <div className="mb-6 flex items-center justify-between">
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">賃金テーブル編集</h1>
